...
namespace pbrt
{
    namespace bdpt
    {
        ...
        inline bool RussianRoulette(glm::vec3 &beta, float eta_scale, const RNG &rng)
        {
    ...
    glm::vec3 BDPTRenderer::RenderPixel(const glm::ivec3 &pixel_coord)
    {
        ...
        auto generateLightSubPath = [&](int max_depth) -> std::vector<bdpt::PathVertex>
        {
            ...
            if (light->GetLightType() == LightType::Area)
            {
                ...
                glm::vec3 origin = light_surface_point + bdpt::SHADOW_EPS * emit_dir;
                glm::vec3 radiance_query_point = origin - emit_dir * bdpt::LIGHT_BACK_EPS;
            ...
            {
                auto light_info = light->SampleLight(mScene.GetCenter(), mScene.GetRadius(), rng, true);
                if (!light_info.has_value())
                {
                    return vertices;
                }
                glm::vec3 dir = -light_info->__direction__;
                glm::vec3 origin = light_info->__lightPoint__ + bdpt::SHADOW_EPS * dir;
            ...
            for (int depth = 0; depth < max_depth; depth++)
            {
                ...
                if (depth >= 2 && bdpt::RussianRoulette(current_beta, eta_scale, rng))
                {
                    break;
                }

                auto sampled = bdpt::SampleDirection(*hit_info, -ray.__direction__, rng);
        ...
        auto generateCameraSubPath = [&](int max_depth, glm::vec3 &radiance) -> std::vector<bdpt::PathVertex>
        {
            ...
            for (int depth = 0; depth < max_depth; depth++)
            {
                ...
                // Russian roulette
                if (depth >= 2 && bdpt::RussianRoulette(beta, eta_scale, rng))
                {
                    break;
                }
        ...
        for (const auto &lv : light_vertices)
        {
            for (const auto &cv : camera_vertices)
            {
                ...
                glm::vec3 shadow_origin = lv.position + dir * bdpt::SHADOW_EPS;
                float shadow_tmin = bdpt::SHADOW_EPS;
                float shadow_tmax = dist - bdpt::SHADOW_EPS;
                ...
                if (bdpt::IsBlack(f_light) || bdpt::IsBlack(f_camera))
                {
                    continue;
                }
                ...
                {
                    weight = bdpt::PowerHeuristic(p_light, p_camera);
                }
