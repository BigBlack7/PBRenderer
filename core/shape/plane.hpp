...
namespace pbrt
{
    ...
    public:
        Plane(const glm::vec3 &point, const glm::vec3 &normal, float radius) : __point__(point), __normal__(glm::normalize(normal)), __radius__(radius)
        {
            glm::vec3 up = glm::abs(normal.y) < 0.99999 ? glm::vec3(0, 1, 0) : glm::vec3(0, 0, -1);
            __xAxis__ = glm::normalize(-glm::cross(up, normal));
            ...
            for (size_t i = 0; i < 8; i++)
            {
                ...
                __bounds__.Expand(corner.x * __xAxis__ + corner.y * __normal__ + corner.z * __zAxis__);
            }
        }
