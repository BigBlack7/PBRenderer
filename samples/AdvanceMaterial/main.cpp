...
int main()
{
    ...
    for (int i = -3; i <= 3; i++)
    {
        scene.AddShape(sphere, new pbrt::DielectricMaterial{{1.f, 1.f, 1.f}, 1.f + 0.2f * (i + 3)}, {0.f, 0.5f, i * 2.f}, {0.8f, 0.8f, 0.8f});
    }
    for (int i = -3; i <= 3; i++)
    {
        glm::vec3 c = pbrt::RGB::GenerateHeatMap((i + 3.f) / 6.f);
        scene.AddShape(sphere, new pbrt::ConductorMaterial{glm::vec3(2.f - c * 2.f), glm::vec3(2.f + c * 3.f)}, {0.f, 2.5f, i * 2.f}, {0.8f, 0.8f, 0.8f});
    }

    scene.AddShape(model, new pbrt::DielectricMaterial{pbrt::RGB(128, 191, 131), 1.8f}, {-5.f, 0.4f, 1.5f}, {2.f, 2.f, 2.f});
    scene.AddShape(model, new pbrt::ConductorMaterial{{0.1f, 1.2f, 1.8f}, {5.f, 2.5f, 2.f}}, {-5.f, 0.4f, -1.5f}, {2.f, 2.f, 2.f});
